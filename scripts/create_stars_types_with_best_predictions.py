#!/usr/bin/env python3
"""
Create stars_types catalog with best-of-three temperature predictions.

Merges stars_types.dat (2.1M eclipsing binaries) with best-of-three
predictions (847k objects). Uses Gaia Teff when available, otherwise
ML predictions with lowest uncertainty.
...
def load_best_predictions(filepath):
    """Load best-of-three predictions."""
    logger.info(f"Loading best-of-three predictions from {filepath}")

    df = pl.read_parquet(filepath)

    logger.info(f"Loaded {len(df):,} predictions")
    logger.info(f"  Mean Teff: {df['teff_best'].mean():.0f} K")
    logger.info(f"  Mean uncertainty: {df['unc_best'].mean():.0f} K")
...
def merge_and_create_catalog(df_stars, df_predictions):
    ...
    - Otherwise use ML prediction (best-of-three)
    ...
    df_merged = df_merged.with_columns([
        pl.when(pl.col('teff_gaia').is_not_null())
          ...
          .otherwise(pl.col('teff_best'))
          .alias('teff_final'),

        # Temperature source indicator
        pl.when(pl.col('teff_gaia').is_not_null())
          .then(pl.lit('Gaia'))
          .when(pl.col('teff_best').is_not_null())
    ...
    from_ml = df_merged['teff_source'].is_in(['teff_only', 'teff_logg', 'teff_cluster']).sum()
...
def save_fits_catalog(df, output_path):
    ...
    # For string columns, ensure consistent dtype by specifying dtype explicitly
    table = Table()
    ...
    table['teff_predicted'] = df['teff_best'].to_numpy().astype(np.float64)
    ...
    table['teff_predicted'].description = 'ML predicted temperature from best-of-three (K)'
...
def create_description_file(df, output_path):
    ...
    from_ml = df['teff_source'].is_in(['teff_only', 'teff_logg', 'teff_cluster']).sum()

    # Quality distribution
    quality_counts = df.group_by('quality_flag').len().sort('quality_flag')

    # Model selection distribution (for ML predictions)
    df_ml = df.filter(pl.col('teff_source').is_in(['teff_only', 'teff_logg', 'teff_cluster']))
    model_counts = df_ml.group_by('teff_source').len().sort('teff_source')

    # Uncertainty statistics (for ML predictions)
    unc_stats_df = df_ml['teff_uncertainty'].describe()
    # Extract statistics from Polars DataFrame
    ...
    description = f"""# stars_types_with_best_predictions.fits - Description
...
table = Table.read('stars_types_with_best_predictions.fits')
...
For each object, three ML models were evaluated:

1. **Teff Only** (Gaia photometry)
   - Features: g, BP, RP, BP-RP
   - Predicts: log10(Teff) → converted to Kelvin
...
**Improvement**: Mean uncertainty reduced by 18.8% compared to single best model (355K → 288K).
...
- **Best-of-Three Methodology**: See `reports/figures/best_uncertainty_ensemble/`
...
def main():
    """Main execution."""
    logger.info("=== Creating stars_types catalog with best-of-three predictions ===\n")

    # Define paths
    base_dir = Path(__file__).parent.parent
    stars_types_file = base_dir / 'data' / 'raw' / 'stars_types.dat'
    predictions_file = base_dir / 'data' / 'processed' / 'teff_predictions_best_of_three.parquet'
    output_fits = base_dir / 'data' / 'processed' / 'stars_types_with_best_predictions.fits'
    output_desc = base_dir / 'data' / 'processed' / 'stars_types_with_best_predictions_DESCRIPTION.txt'
