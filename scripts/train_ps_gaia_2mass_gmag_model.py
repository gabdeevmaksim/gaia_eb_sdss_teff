#!/usr/bin/env python3
"""
Train Random Forest model on Pan-STARRS + Gaia colors + Gaia g magnitude.

This script trains a temperature prediction model using 5 features:
- Pan-STARRS colors: g-r, r-i, i-z
- Gaia color: BP-RP
- Gaia magnitude: phot_g_mean_mag

This model includes magnitude information and may exhibit distance/brightness bias.
Compare with color-only unified model to assess magnitude impact.

Usage:
    # Train with default parameters
    nohup python scripts/train_panstarrs_gaia_gmag_model.py > train_ps_gaia_gmag.log 2>&1 &

    # Custom hyperparameters
    python scripts/train_panstarrs_gaia_gmag_model.py --n-estimators 500 --max-depth 25
...
def load_training_data(config):
    """Load Pan-STARRS + Gaia + 2MASS with g magnitude training dataset."""
    input_file = config.get_path('processed') / 'ps_gaia_2mass_gmag_training.parquet'

    if not input_file.exists():
        logger.error(f"Training data not found: {input_file}")
        logger.error("Please run: python scripts/create_ps_gaia_2mass_gmag_dataset.py")
...
def prepare_features_and_target(df):
    ...
    Features: 7 colors + 1 magnitude = 8 total
    Target: teff_gspphot
    ...
    tuple
        (X, y, feature_names)
    """
    # Define features (8 total: 7 colors + 1 magnitude)
    features = [
        'bp_rp',            # Gaia color
        'g_r_color',        # Pan-STARRS colors
        'r_i_color',
        'i_z_color',
        'j_h_color',        # 2MASS NIR colors
        'h_k_color',
        'j_k_color',
        'phot_g_mean_mag'   # Gaia magnitude
    ]

    logger.info(f"\nPreparing features:")
    logger.info(f"  Total features: {len(features)}")
    logger.info(f"  Optical colors: 4 (bp_rp, g-r, r-i, i-z)")
    logger.info(f"  NIR colors: 3 (j-h, h-k, j-k)")
    logger.info(f"  Magnitudes: 1 (phot_g_mean_mag)")

    for i, feat in enumerate(features, 1):
        feat_type = "magnitude" if "mag" in feat else "color"
        logger.info(f"    {i}. {feat:<20} ({feat_type})")
...
def save_model_and_metadata(results, config, hyperparameters, dataset_info):
    ...
    model_id = f"rf_ps_gaia_2mass_gmag_{timestamp}"
    ...
    metadata = {
        ...
        'dataset_type': 'ps_gaia_2mass_with_gmag',
    ...
    with open(summary_file, 'w') as f:
        f.write(f"PAN-STARRS + GAIA + 2MASS (with g magnitude) MODEL\n")
        f.write(f"=" * 70 + "\n\n")
        ...
        f.write(f"  Features: {len(results['feature_names'])} (7 colors + 1 magnitude)\n\n")

        f.write(f"FEATURES (8 total):\n")
        f.write(f"  Optical colors (4):\n")
        f.write(f"    - bp_rp (Gaia BP-RP)\n")
        f.write(f"    - g_r_color, r_i_color, i_z_color (Pan-STARRS)\n")
        f.write(f"  NIR colors (3):\n")
        f.write(f"    - j_h_color, h_k_color, j_k_color (2MASS)\n")
        f.write(f"  Magnitude (1):\n")
        f.write(f"    - phot_g_mean_mag (Gaia g-band)\n\n")
        ...
        f.write(f"\n" + "=" * 70 + "\n")
        f.write(f"NOTES:\n")
        f.write(f"- This model includes Gaia g magnitude\n")
        f.write(f"- May exhibit magnitude/distance bias\n")
        f.write(f"- Compare with color-only unified model to assess magnitude impact\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train RF model on Pan-STARRS + Gaia + 2MASS colors + Gaia g magnitude',
    ...
    logger.info("=" * 80)
    logger.info("TRAIN PAN-STARRS + GAIA + 2MASS (WITH G MAGNITUDE) MODEL")
    logger.info("=" * 80)
    ...
    logger.info(f"  2. Compare with color-only unified model")
