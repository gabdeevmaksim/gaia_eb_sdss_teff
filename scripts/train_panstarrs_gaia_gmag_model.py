...
def load_training_data(config):
    """Load Pan-STARRS + Gaia with g magnitude training dataset."""
    input_file = config.get_path('processed') / 'panstarrs_gaia_with_gmag_training.parquet'

    if not input_file.exists():
        logger.error(f"Training data not found: {input_file}")
        logger.error("Please run: python scripts/create_panstarrs_gaia_with_gmag_dataset.py")
...
def prepare_features_and_target(df):
    ...
    Features: 4 colors + 1 magnitude = 5 total
    Target: teff_gspphot
    ...
    logger.info(f"  Colors: {len([f for f in features if 'color' in f or 'bp_rp' in f])}")
    logger.info(f"  Magnitudes: {len([f for f in features if 'mag' in f])}")
...
def save_model_and_metadata(results, config, hyperparameters, dataset_info):
    ...
    model_id = f"rf_ps_gaia_gmag_{timestamp}"
    ...
    metadata = {
        ...
        'dataset_type': 'panstarrs_gaia_with_gmag',
    ...
    with open(summary_file, 'w') as f:
        f.write(f"PAN-STARRS + GAIA (with g magnitude) MODEL\n")
        f.write(f"=" * 70 + "\n\n")
        ...
        f.write(f"  Features: {len(results['feature_names'])} (4 colors + 1 magnitude)\n\n")

        f.write(f"FEATURES (5 total):\n")
        f.write(f"  Colors (4):\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train RF model on Pan-STARRS + Gaia colors + Gaia g magnitude',
    ...
    logger.info("=" * 80)
    logger.info("TRAIN PAN-STARRS + GAIA (WITH G MAGNITUDE) MODEL")
    logger.info("=" * 80)
