#!/usr/bin/env python3
"""
Create standardized validation plots for Gaia G + BP-RP Engineered model.
...
def main():
    ...
    logger.info("="*80)
    logger.info("CREATE GAIA G + BP-RP ENGINEERED VALIDATION PLOTS")
    logger.info("="*80)

    # Find most recent gaia_g_bprp_engineered model
    model_files = sorted(models_dir.glob('rf_gaia_g_bprp_engineered_*.pkl'))
    if not model_files:
        raise FileNotFoundError("No Gaia G + BP-RP Engineered model found!")

    model_path = model_files[-1]
    MODEL_ID = model_path.stem
    MODEL_NAME = 'Gaia G + BP-RP Engineered Model'
    SUBDIR = 'gaia_g_bprp_engineered_validation'
    ...
    # Load training data
    import polars as pl
    df_full = pl.read_parquet(data_dir / 'gaia_g_bprp_engineered_train.parquet')
    ...
    # 5. Color distributions (just bp_rp for simplicity, since it's the base color)
