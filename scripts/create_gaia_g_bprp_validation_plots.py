#!/usr/bin/env python3
"""
Create standardized validation plots for Gaia G + BP-RP model.
...
def main():
    ...
    logger.info("="*80)
    logger.info("CREATE GAIA G + BP-RP VALIDATION PLOTS")
    logger.info("="*80)

    # Find most recent gaia_g_bprp model
    model_files = sorted(models_dir.glob('rf_gaia_g_bprp_*.pkl'))
    if not model_files:
        raise FileNotFoundError("No Gaia G + BP-RP model found!")

    model_path = model_files[-1]
    MODEL_ID = model_path.stem
    MODEL_NAME = 'Gaia G + BP-RP Model'
    SUBDIR = 'gaia_g_bprp_validation'
    ...
    # Load training data
    import polars as pl
    df_full = pl.read_parquet(data_dir / 'gaia_g_bprp_train.parquet')
