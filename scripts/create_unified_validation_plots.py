#!/usr/bin/env python3
"""
Create standardized validation plots for Unified (Pan-STARRS + Gaia) model.
...
def main():
    ...
    logger.info("="*80)
    logger.info("CREATE UNIFIED (PAN-STARRS + GAIA) VALIDATION PLOTS")
    logger.info("="*80)

    MODEL_ID = 'rf_unified_engineered_20251016_112332'
    MODEL_NAME = 'Unified Pan-STARRS + Gaia Model'
    SUBDIR = 'unified_validation'
    ...
    # Training data
    df_train = pd.read_parquet(processed_dir / 'eb_unified_features_engineered_train.parquet')
    ...
    # Color distributions (5 base colors + 1 repeat for 2x3 layout)
    colors_to_compare = ['bp_rp', 'g_r_color', 'r_i_color', 'i_z_color', 'B_V_color']
    color_labels = ['BP-RP', 'g-r', 'r-i', 'i-z', 'B-V']
    ...
    # Feature importance (top 20 engineered features)
