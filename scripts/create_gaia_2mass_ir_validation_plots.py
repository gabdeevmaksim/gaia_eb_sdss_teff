#!/usr/bin/env python3
"""
Create standardized validation plots for Gaia + 2MASS IR model.
...
def main():
    ...
    logger.info("="*80)
    logger.info("CREATE GAIA + 2MASS IR VALIDATION PLOTS")
    logger.info("="*80)

    # Find most recent gaia_2mass_ir model
    model_files = sorted(models_dir.glob('rf_gaia_2mass_ir_*.pkl'))
    if not model_files:
        raise FileNotFoundError("No Gaia + 2MASS IR model found!")

    model_path = model_files[-1]
    MODEL_ID = model_path.stem
    MODEL_NAME = 'Gaia + 2MASS IR Model'
    SUBDIR = 'gaia_2mass_ir_validation'
    ...
    # Load training data
    import polars as pl
    df_full = pl.read_parquet(data_dir / 'gaia_2mass_ir_train.parquet')
