#!/usr/bin/env python3
"""
Train Random Forest temperature regression model using engineered G + BP-RP features.

Features: 17 engineered features from phot_g_mean_mag and bp_rp
Target: teff_gspphot (Gaia GSP-Phot effective temperature)

This model tests whether feature engineering can improve performance
over the basic 2-feature model.
"""
...
def main():
    ...
    model_id = f'rf_gaia_g_bprp_engineered_{timestamp}'

    print("=" * 80)
    print("Training Random Forest: Gaia G + BP-RP Engineered Features")
    print("=" * 80)
    ...
    df = pl.read_parquet(data_dir / 'gaia_g_bprp_engineered_train.parquet')
    print(f"   Loaded {len(df):,} training samples")

    # Identify feature columns
    FEATURES = [c for c in df.columns if c not in ['source_id', 'teff_gspphot']]
    ...
    sample_features = FEATURES[:5]  # Show first 5
    for i, feat in enumerate(sample_features):
        feat_idx = FEATURES.index(feat)
        print(f"     {feat:20s}: mean={X[:, feat_idx].mean():.3f}, std={X[:, feat_idx].std():.3f}, "
              f"min={X[:, feat_idx].min():.3f}, max={X[:, feat_idx].max():.3f}")
    ...
    for feat, importance in sorted_features[:10]:
        print(f"   {feat:25s}: {importance:.4f} ({importance*100:.1f}%)")
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("=" * 80 + "\n\n")
        f.write("FEATURES:\n")
        f.write(f"  Base: phot_g_mean_mag, bp_rp\n")
        f.write(f"  Engineered: {len(FEATURES)} total features\n")
        f.write(f"    - Polynomial terms\n")
        f.write(f"    - Interaction terms\n")
        f.write(f"    - Logarithmic transforms\n")
        f.write(f"    - Temperature-dependent features\n\n")
        f.write("DATASET:\n")
        ...
        for feat, importance in sorted_features[:10]:
            f.write(f"  {feat:25s}: {importance:.4f} ({importance*100:.1f}%)\n")
        f.write("\n")
    ...
    print(f"\nComparison to basic model (2 features):")
    print(f"  Feature count: 2 → {len(FEATURES)} ({len(FEATURES)/2:.1f}x increase)")
